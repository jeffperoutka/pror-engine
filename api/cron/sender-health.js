...
async function run() {
  ...
  try {
    ...
    const base = new Airtable({ apiKey: (process.env.AIRTABLE_API_KEY || process.env.AIRTABLE_PAT || '').trim() }).base(
      (process.env.AIRTABLE_BASE || process.env.AIRTABLE_BASE_ID || '').trim()
    );
